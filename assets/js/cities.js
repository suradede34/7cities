...
let foodCities = cities.filter(city => city.type === "food");
let historicalCities = cities.filter(city => city.type === "place");

foodCities.forEach(city => {
    foodList.innerHTML += `<li>${city.city}: ${city.famousFor}</li>`;
});

historicalCities.forEach(city => {
    historicalList.innerHTML += `<li>${city.city}: ${city.famousFor}</li>`;
});
